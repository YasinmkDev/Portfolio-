...
(function(){
  ...
  document.addEventListener('mouseleave', ()=>{cursor.classList.add('cursor--hidden');outline.classList.add('cursor--hidden');});
  document.addEventListener('mouseenter', ()=>{cursor.classList.remove('cursor--hidden');outline.classList.remove('cursor--hidden');});

  const interactive = 'a, button, input, textarea, [role="button"]';
  document.addEventListener('pointerover', e=>{if(e.target.closest(interactive)){cursor.classList.add('cursor--expand');outline.classList.add('cursor-outline--hover')}})
  document.addEventListener('pointerout', e=>{if(e.target.closest(interactive)){cursor.classList.remove('cursor--expand');outline.classList.remove('cursor-outline--hover')}})

  document.addEventListener('pointerdown', e=>{
    ripple.style.left = e.clientX+'px';ripple.style.top = e.clientY+'px';
    ripple.style.width='16px';ripple.style.height='16px';ripple.style.opacity='1';
    ripple.style.transition='none';ripple.style.transform='translate3d(-50%,-50%,0) scale(0.2)';
    requestAnimationFrame(()=>{
      ripple.style.transition='opacity 360ms ease, transform 480ms ease';
      ripple.style.transform='translate3d(-50%,-50%,0) scale(20)';
